# This script is used to prepare the attribute data for the spatial analysis of India
#The necessary libraries will involved importing and scraping data from Wikipedia
...
tidy_pop <- pop %>%
  ...
  mutate(
    ...
    density_km2 = sub("/.*", "", density_km2) %>% 
      str_replace_all(",", "") %>%
      as.numeric(),
    state_ut = replace(state_ut, state_ut == 'Manipur??', 'Manipur')
  ) %>% 
  arrange(state_ut)
